...
export default async function Page({
  ...
  return (
    <main>
      ...
      {/* MAR 환율 누락 알림 */}
      {pnlTrend.missingMarDates.length > 0 && (
        <div className="mb-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/30 px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300">
              <span className="font-semibold">MAR 환율 누락</span>
              <span className="ml-1 text-amber-600 dark:text-amber-400">
                — {pnlTrend.missingMarDates.map((d) => `${d.slice(4,6)}/${d.slice(6,8)}`).join(', ')} (USD PnL 부정확 가능)
              </span>
            </p>
          </div>
        </div>
